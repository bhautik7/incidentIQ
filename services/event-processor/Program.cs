...
// The log explorer's retention window. Without this the raw table is the only
// one in the schema that grows with traffic and never stops.
builder.Services.Configure<RawLogRetentionOptions>(
    builder.Configuration.GetSection(RawLogRetentionOptions.SectionName));
builder.Services.AddHostedService<RawLogRetentionService>();
